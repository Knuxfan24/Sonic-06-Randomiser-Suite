	...
	public class DDSImage
    {
		...
		private Bitmap readBlockImage(byte[] data, int w, int h) {
			switch (header.ddspf.dwFourCC) {
                ...
                case FOURCC_DXT3:
                    return UncompressDXT3(data, w, h);
                case FOURCC_DXT5:
        ...
        private Bitmap UncompressDXT3(byte[] data, int w, int h) {
            ...
            using (MemoryStream ms = new MemoryStream(data)) {
                using (BinaryReader r = new BinaryReader(ms)) {
                    for (int j = 0; j < h; j += 4) {
                        for (int i = 0; i < w; i += 4) {
                            DecompressBlockDXT3(i, j, r.ReadBytes(16), res);
                        }
                    }
                }
            }
        ...
        void DecompressBlockDXT3(int x, int y, byte[] blockStorage, Bitmap image) {
            byte alpha3 = blockStorage[3];
            byte alpha7 = blockStorage[7];
            ...
            for (int j = 0; j < 4; j++) {
                for (int i = 0; i < 4; i++) {
                    alpha7 = (byte)(alpha3 | (alpha3 << 4));

                    byte colorCode = (byte)((code >> 2 * (4 * j + i)) & 0x03);
                    Color finalColor = new Color();
                    switch (colorCode) {
                        case 0:
                            finalColor = Color.FromArgb(alpha7, r0, g0, b0);
                            break;
                        case 1:
                            finalColor = Color.FromArgb(alpha7, r1, g1, b1);
                            break;
                        case 2:
                            finalColor = Color.FromArgb(alpha7, (2 * r0 + r1) / 3, (2 * g0 + g1) / 3, (2 * b0 + b1) / 3);
                            break;
                        case 3:
                            finalColor = Color.FromArgb(alpha7, (r0 + 2 * r1) / 3, (g0 + 2 * g1) / 3, (b0 + 2 * b1) / 3);
                            break;
                    }
                    image.SetPixel(x + i, y + j, finalColor);
                }
